...
def standard_model_inversion(
    ...
    for i in range(alpha_iterations):
        ...
        cost = 1.0 - probabilities[0, target_class]
    ...
    return {
        "best_x": best_x.detach(), "cost_history": cost_history, "best_cost":    best_cost, "time_taken": etime - stime
        }

# --- Algorithm 2: Sliding Model Inversion ---

def sliding_model_inversion(
    model: nn.Module,
    target_class: int,
    length_l: int,
    windowsize_w: int,
    ...
    """
    Implements the Sliding Model Inversion (SMI) attack (Algorithm 2).

    This extends standard MI by iteratively inverting overlapping 
    chunks of an audio sample[cite: 11, 93].

    Args:
        model: The trained SincNet model (or any nn.Module) to attack.
        target_class: The integer index of the speaker class to invert.
        length_l: The total length 'l' of the audio sample to generate[cite: 77].
        windowsize_w: The window size 'w' [cite: 77] (e.g., 3200 [cite: 145]).
        stride_s: The stride 's' [cite: 77] (e.g., 500 [cite: 103, 145]).
        alpha_iterations: Max iterations for *each window*.
        lambda_lr: Learning rate for *each window*. (Paper found 0.005 
                   for Laplace worked well)[cite: 218].
        gamma_min_cost: Early stopping cost threshold.
        beta_patience: Early stopping patience.
        init_type: The initialization strategy for the main vector.

    Returns:
        The inverted audio sample as a torch.Tensor. The vector is
        trimmed as described in the paper[cite: 89, 109].
    """
    stime = time.perf_counter()
    device = next(model.parameters()).device
    model.eval()

    # 1. Initialize a random input vector of length 'l' [cite: 79]
    inverted_vector = create_initial_vector(length_l, init_type).to(device)
    
    # We will modify this vector in-place
    inverted_vector = inverted_vector.clone().detach()

    #print(f"[Sliding MI] Attacking class {target_class} with w={windowsize_w}, s={stride_s}...")
    cost_history: List[float] = []
    
    # 2. Iterate 'k' according to stride 's' [cite: 80]
    for k in range(0, length_l - windowsize_w + 1, stride_s):
        
        # 3. Get the current window. This window consists of previously
        #    inverted data and new random data[cite: 101].
        window_tensor = inverted_vector[:, k : k + windowsize_w].clone().detach()
        window_tensor.requires_grad_()
        
        # 4. Setup optimizer *for this window only*
        optimizer = optim.SGD([window_tensor], lr=lambda_lr)
        
        best_window_cost = float('inf')
        best_window_tensor = window_tensor.clone()
        cost_history: List[float] = []
        
        # 5. Run the standard MI gradient descent loop (for 'alpha' iterations)
        #    on this specific window [cite: 82]
        for i in range(alpha_iterations):
            optimizer.zero_grad()
            
            # Note: The model is run *only* on the window
            probabilities = model(window_tensor)
            #print(k, probabilities[0, target_class])
            cost = 1.0 - probabilities[0, target_class]
            ...
            if current_cost < best_window_cost:
                best_window_cost = current_cost
                best_window_tensor = window_tensor.clone().detach()

            # 6. Check early stopping conditions [cite: 84, 86]
            if current_cost <= gamma_min_cost:
                break
            
            if i > beta_patience:
                if current_cost >= max(cost_history[-beta_patience:]):
                    break # Cost not improving
        
        # 7. Replace the vector's part with the inverted data [cite: 88, 118, 120]
        inverted_vector[:, k : k + windowsize_w] = best_window_tensor.detach()
        
        #print(f"Window k={k}: Best cost = {best_window_cost:.4f}")

    # 8. Return the trimmed vector [cite: 89, 109]
    #start_trim = windowsize_w // 2
    #end_trim = length_l - (windowsize_w // 2)
    etime = time.perf_counter()
    return {
        "best_x": inverted_vector.detach(), "cost_history": cost_history, "time_taken": etime - stime
        }
...
# Attack parameters
WINDOW_SIZE_W = wlen       # 200ms of audio [cite: 145]
STRIDE_S = 2000             # 30ms stride [cite: 145]
TOTAL_LENGTH_L = wlen+4000     # Arbitrary total length (e.g., 1 sec)
...
def run_attack(speaker_class: int, idx:int, init_type: str = 'zeros'):
    ...
    initial_x0 = create_initial_vector(
        length=WINDOW_SIZE_W, 
        init_type=init_type
    ).to(DEVICE)

    inverted_sample_std = standard_model_inversion(
        model=my_model,
        target_class=speaker_class,
        input_vector=initial_x0,
        alpha_iterations=1000,
        lambda_lr=0.001,
        gamma_min_cost= 1e-8, # [cite: 218]
        beta_patience=200 # Increased patience
    )

    #print(f"Standard MI complete. Inverted sample shape: {inverted_sample_std.shape}")

    # --- 4. Run Sliding MI Attack (Algorithm 2) ---
    #print("\n--- Running Sliding MI (Algorithm 2) ---")

    inverted_sample_sliding = sliding_model_inversion(
        model=my_model,
        target_class=speaker_class,
        length_l=TOTAL_LENGTH_L,
        windowsize_w=WINDOW_SIZE_W,
        stride_s=STRIDE_S,
        alpha_iterations=1000, # Iterations *per window*
        lambda_lr=0.001,
        beta_patience=200,
        gamma_min_cost= 1e-8,
        init_type=init_type
    ...
    torch.save(inverted_sample_std, f"inverted_samples/{init_type}/{speaker_class}/{idx}/inverted_sample_std.pt")
    torch.save(inverted_sample_sliding, f"inverted_samples/{init_type}/{speaker_class}/{idx}/inverted_sample_sliding.pt")
    return True

TYPE_INITS = ['zeros', 'white_noise_tanh', 'laplace']
SPEAKERS = list(range(35))  # Assuming 35 speaker classes
NUM_TRYS = range(3)
