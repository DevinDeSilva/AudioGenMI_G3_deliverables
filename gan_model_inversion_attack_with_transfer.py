...
gen_weight = torch.load("/home/desild/work/academic/sem3/TrustworthyML-assignment/tacotron2/vctk/models/HIFIGAN_ADV/20251203_095647/checkpoint.pth")
...
def gen_inversion_attack(
    ...
    print(f"[GAN MI] Attacking class {target_class} for {alpha_iterations} iterations...")
...
def standard_model_inversion(
    ...
    etime = time.perf_counter()
    return {
        "best_x": best_x.detach(), "cost_history": cost_history, "best_cost":    best_cost, "time_taken": etime - stime + add_timing
        }
...
def run_attack(speaker_class: int, idx:int, init_type: str = 'zeros'):
    ...
    inverted_sample_gan_std = standard_model_inversion(
        model=my_model,
        target_class=speaker_class,
        input_vector=inverted_sample_gan['best_audio'],
        alpha_iterations=1000,
        lambda_lr=0.001,
        gamma_min_cost= 1e-8, # [cite: 218]
        beta_patience=200, # Increased patience
        add_timing=inverted_sample_gan['time_taken'],
    )

    #print(f"Sliding MI complete. Inverted sample shape: {inverted_sample_sliding.shape}")

    torch.save(inverted_sample_gan_norm, f"inverted_samples/{init_type}/{speaker_class}/{idx}/inverted_sample_gan-normaud-transfer.pt")
    torch.save(inverted_sample_gan, f"inverted_samples/{init_type}/{speaker_class}/{idx}/inverted_sample_gan-transfer.pt")
    torch.save(inverted_sample_gan_std, f"inverted_samples/{init_type}/{speaker_class}/{idx}/inverted_sample_gan-std-transfer.pt")
    return True
